...
class BertEncoder(nn.Module):
    ...
    def forward(self, src):
        ...
        output = self.fc(output)  # (N, S, D_LATENT)
        output = torch.permute(output, (0, 2, 1))  # (N, D_LATENT, S)
        output = self.pool(output)  # (N, D_LATENT, 1)
        output = torch.squeeze(output, dim=2)  # (N, D_LATENT)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--d_latent", type=int, default=8, help="Dimension of the latent representation."
    )
    parser.add_argument(
